            ...
            <xsl:for-each select="/project/file/*">
                ...
                <xsl:if test="count(./*/docblock/tag[@name='api']) > 0">
                    ...
                    <li class="closed">
                        ...
                        <xsl:variable name="rootClass" select="../@generated-path" />
                        ...
                        <xsl:for-each select="./*/docblock/tag[@name='api']">
                            ...
                            <span>
                            <xsl:choose>
                                <xsl:when test="count(../tag[@name='url'])=0">
                                    no url tag for docblock
                                </xsl:when>
                                <xsl:otherwise>
                                    <xsl:for-each select="../tag[@name='url']">
                                        <!-- show the first link -->
                                        <xsl:if test="(position( )) = 1">
                                            <a href="{$root}{$rootClass}#{../../full_name}" target="content" class="api-url">
                                                <span class="api-method"><xsl:value-of select="../../docblock/tag[@name='method']/@description" /></span>
                                                <span class="api-uri"><xsl:value-of select="../../docblock/tag[@name='url']/@description" /></span>
                                            </a>
                                        </xsl:if>
                                    </xsl:for-each>
                                </xsl:otherwise>
